...
class CNN_Attention():
    def __init__(self, s, w, l2_reg, model_type, num_features, d0=300, di=50, num_classes=2, num_layers=2):
        ...
        def convolution(name_scope, x, d, reuse):
            with tf.name_scope(name_scope + "-conv"):
                with tf.variable_scope("conv") as scope:
                    ...
                    # [batch, di, s+w-1, 1]
                    self.conv_trans = tf.transpose(conv, [0, 3, 2, 1], name="conv_trans")
                    return self.conv_trans
        ...
        # self.cost = -tf.reduce_sum(tf.one_hot(self.y, depth = 2) * tf.clip_by_value(tf.nn.softmax(self.estimation),1e-10,1.0))

        # self.cost = tf.reduce_sum(tf.square(tf.subtract(self.estimation, tf.one_hot(self.y, depth = 2))))
        
        self.cost = tf.add(
            -tf.reduce_sum(tf.one_hot(self.y, depth = 2) * tf.log(tf.nn.softmax(self.estimation) + 1e-10)),
            tf.reduce_sum(tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)),
            name="cost")

        # self.cost = tf.add(
        #     tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.estimation, labels=self.y)),
        #     tf.reduce_sum(tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)),
        #     name="cost")
